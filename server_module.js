...
router.get('/', (req, res) => {
    try {
        const channels = JSON.parse(fs.readFileSync('./ids_recordplus.json', 'utf8'));
        ...
        <head>
            ...
            <title>UNIBOX - RecordPlus</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
            <style>
                body { background: #1a001a; color: #fff; font-family: sans-serif; }
                .topo { background: #330033; padding: 15px 0; margin-bottom: 20px; border-bottom: 3px solid #bf40bf; }
                .card { background: #260026; border: 1px solid #444; transition: 0.3s; color: #fff; }
                .card:hover { border-color: #bf40bf; transform: translateY(-5px); }
                .card-title { font-size: 0.9rem; font-weight: bold; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 5px; }
                .online { font-size: 0.75rem; color: #bf40bf; margin-bottom: 10px; }
                .btn-watch { background: #bf40bf; color: #fff; width: 100%; font-weight: bold; margin-bottom: 5px; }
                .btn-watch:hover { background: #993399; color: #fff; }
                .btn-copy { background: #333; color: #fff; width: 100%; font-size: 0.8rem; }
            </style>
        </head>
        <body>
        <div class="topo"><div class="container d-flex justify-content-between align-items-center">
            <h4 class="m-0">📺 UNIBOX <span style="color:#bf40bf">RECORDPLUS</span></h4>
...
router.get('/baixar-m3u', (req, res) => {
    try {
        const canais = JSON.parse(fs.readFileSync('./ids_recordplus.json', 'utf8'));
        const token = process.env.AUTH_TOKEN || 'unibox';
        const host = req.get('host');
        const baseUrl = req.baseUrl;
        let m3u = "#EXTM3U\n";
        canais.forEach((ch, index) => {
            ...
            m3u += `#EXTINF:-1 tvg-logo="${ch.logo}" group-title="Record",${ch.nome}\n${link}\n`;
        });
        res.setHeader('Content-Disposition', 'attachment; filename=record.m3u');
        res.send(m3u);
    } catch (e) { res.end(); }
});
...
function fetchStreamUrl(canal) {
    return new Promise((resolve) => {
        const args = ['--get-url', '--format', 'best', '--no-warnings', `https://www.recordplus.com/live-channel/${canal.id}`];
