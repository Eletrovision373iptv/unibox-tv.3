...
router.get('/', (req, res) => {
    try {
        const channels = JSON.parse(fs.readFileSync('./ids_redetv.json', 'utf8'));
        ...
        <head>
            ...
            <title>UNIBOX - RedeTV!</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
            <style>
                body { background: #1a1a1a; color: #fff; font-family: sans-serif; }
                .topo { background: #000; padding: 15px 0; margin-bottom: 20px; border-bottom: 3px solid #00aeef; }
                .card { background: #222; border: 1px solid #333; transition: 0.3s; color: #fff; }
                .card:hover { border-color: #00aeef; transform: translateY(-5px); }
                .card-title { font-size: 0.9rem; font-weight: bold; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 5px; }
                .online { font-size: 0.75rem; color: #00aeef; margin-bottom: 10px; }
                .btn-watch { background: #00aeef; color: #fff; width: 100%; font-weight: bold; margin-bottom: 5px; border: none; }
                .btn-watch:hover { background: #008cc0; color: #fff; }
            </style>
        </head>
        <body>
        <div class="topo"><div class="container d-flex justify-content-between align-items-center">
            <h4 class="m-0">📺 UNIBOX <span style="color:#00aeef">RedeTV!</span></h4>
            <a href="${baseUrl}/baixar-m3u" class="btn btn-info fw-bold btn-sm">M3U</a>
        </div></div>
...
router.get('/baixar-m3u', (req, res) => {
    try {
        const canais = JSON.parse(fs.readFileSync('./ids_redetv.json', 'utf8'));
        const token = process.env.AUTH_TOKEN || 'unibox';
        const host = req.get('host');
        const baseUrl = req.baseUrl;
        let m3u = "#EXTM3U\n";
        canais.forEach((ch, index) => {
            ...
            m3u += `#EXTINF:-1 tvg-logo="${ch.logo}" group-title="RedeTV",${ch.nome}\n${link}\n`;
        });
        res.setHeader('Content-Disposition', 'attachment; filename=redetv.m3u');
        res.send(m3u);
    } catch (e) { res.end(); }
});
...
function fetchStreamUrl(canal) {
    return new Promise((resolve) => {
        const args = ['--get-url', '--format', 'best', '--no-warnings', canal.url];
